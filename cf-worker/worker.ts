...
<style>
*{box-sizing:border-box}body{margin:0;background:#06101b;color:#edf5ff;font:14px system-ui,sans-serif}main{max-width:980px;margin:0 auto;padding:28px}
...
pre{white-space:pre-wrap;word-break:break-word;background:#050d16;border:1px solid #1a3048;padding:12px;border-radius:10px;max-height:340px;overflow:auto}
.ok{color:#83dfbd}.warn{color:#efc77d}.bad{color:#ef9a9a}@media(max-width:720px){.grid{grid-template-columns:1fr 1fr}}
</style></head>
<body><main>
...
<div class="grid" id="summary"></div><p id="state"></p><pre id="output">Aguardando execução.</pre></div>
</main><script>
let sessionToken='';
...
async function load(){const data=await api('/api/admin/bootstrap?runtime=cloudflare',{sessionToken});const select=qs('target');select.innerHTML='';for(const t of data.certificationTargets||[]){const o=document.createElement('option');o.value=t.id;o.textContent=t.name+' · '+t.certification.summary.proved+'/'+t.certification.summary.applicable;select.appendChild(o)}renderSummary(data.summary);return data}
