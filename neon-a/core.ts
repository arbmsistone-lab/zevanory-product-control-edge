import { db, error, json, router, secrets } from './portable-sdk.ts';
...
async function bootstrapFromLegacy(pin: string) {
  const legacyBase = 'https://arbm-control-senior-d2xvhh.v2.appdeploy.ai';
  let sessionToken = '';
  try {
    const login = await fetch(legacyBase + '/api/pin/login', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ pin }),
    });
    if (!login.ok) return false;
    const auth = await login.json() as { sessionToken?: string };
    sessionToken = String(auth.sessionToken || '');
    if (!sessionToken) return false;

    const boot = await fetch(legacyBase + '/api/admin/bootstrap', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ sessionToken }),
    });
    if (!boot.ok) return false;
    const data = await boot.json() as any;
    const hash = await pinFingerprint(pin);
    await secrets.writeSecret('ADMIN_PIN_HASH', hash, true);

    const seedIfEmpty = async (table: string, rows: any[]) => {
      const current = await db.list<any>(table, { limit: 1 });
      if (current.items.length > 0 || !Array.isArray(rows) || rows.length === 0) return;
      const cleaned = rows.map(row => {
        const { id, certification, commercialReady, blockers, auditOverall, auditStatus, ...record } = row || {};
        return record;
      });
      if (cleaned.length > 0) await db.add(table, cleaned);
    };

    await seedIfEmpty(SYSTEMS, data?.dashboard?.systems || []);
    await seedIfEmpty(AUDITS, data?.dashboard?.audits || []);
    await seedIfEmpty(IMPROVEMENTS, data?.dashboard?.improvements || []);
    await seedIfEmpty(INCIDENTS, data?.dashboard?.incidents || []);
    await seedIfEmpty(productTable(), data?.products || []);
    const engine = await db.list<any>(ENGINE, { limit: 1 });
    if (engine.items.length === 0) await db.add(ENGINE, [{ lastRun: data?.dashboard?.lastEngineRun || new Date().toISOString() }]);
    return true;
  } catch {
    return false;
  } finally {
    if (sessionToken) {
      try {
        await fetch(legacyBase + '/api/pin/logout', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ sessionToken }),
        });
...
async function pinLogin(pin: unknown) {
  ...
  let configuredHash = String(await secrets.readSecret('ADMIN_PIN_HASH') || '').trim();
  if (!/^[a-f0-9]{64}$/.test(configuredHash)) {
    const migrated = await bootstrapFromLegacy(candidate);
    if (!migrated) return { ok: false, status: 401, message: 'PIN incorreto.' };
    configuredHash = String(await secrets.readSecret('ADMIN_PIN_HASH') || '').trim();
  }
  if (!/^[a-f0-9]{64}$/.test(configuredHash)) throw new Error('ADMIN_PIN_HASH_invalid');
  const fingerprint = configuredHash;
  const candidateHash = await pinFingerprint(candidate);
...
export const handler = router({
  'GET /api/_auth_diagnostic': [async () => {
    ...
    let tempSessionId = '';
    try {
      const configuredHash = String(await secrets.readSecret('ADMIN_PIN_HASH') || '').trim();
      const secretValid = /^[a-f0-9]{64}$/.test(configuredHash);
      if (!secretValid) return json({ secretValid: false, locked: false, sessionRoundtrip: false, bootstrapOk: false, stage: 'migration_pending' });
  ...
  'POST /api/products/create': [async ctx => {
    ...
    const portableId = String((body as any).portableId || '').trim();
    let id = '';
    if (/^[a-zA-Z0-9_-]{16,80}$/.test(portableId)) {
      const [updatedId] = await db.update(productTable(), [{ id: portableId, record: product }]);
      id = updatedId || '';
    } else {
      const [addedId] = await db.add(productTable(), [product]);
      id = addedId || '';
    }
